...
def describe_node(
    ...
    node: dict[str, Any] = {
        ...
        "size": size,
        "preview": _container_preview(value, th),
        # Rich key:value summary so a non-root container node previews the same
        # way the top-level row does (values one level in, deeper stays shape).
        "summary": row_summary(value, th),
    }
    if key is not None:
        node["key"] = key
